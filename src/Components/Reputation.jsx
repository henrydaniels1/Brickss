...
export const Rep = () => {
 ...
 useEffect(() => {
  const currentSectionRef = sectionRef.current; // Capture the current ref value
  ...
  if (currentSectionRef) {
    observer.observe(currentSectionRef); // Start observing the section
  }

  return () => {
    if (currentSectionRef) {
      observer.unobserve(currentSectionRef); // Stop observing when the component unmounts
    }
  };
}, []);
  ...
  return (
    <div
      ...
      <div
        ...
        <p className='lg:text-4xl reveal2 md:text-3xl text-teal-900 text-2xl'>
          Our Services
        </p>
      </div>
          ...
          >
            <div>
              ...
              <div className='flex items-center justify-center w-16 h-16 mb-4 rounded-full bg-indigo-50'>
                {icons[index]}{' '}
                {/* Display the icon corresponding to the current card */}
              </div>
              {/* Service Title */}
